...
public class Plunger : CameraUser
{
    ...
    [SerializeField] private float _cameraPrepareLaunchAnimDuration;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.CompareTag("Player") && !_isTargetInside)
        {
            ...
            BeginCameraUseWithFixedPoints(temp, temp.CameraPosition, _cameraPoint.position, _cameraPoint.rotation, false, false, _cameraPrepareLaunchAnimDuration);
            UpdatePlayerPosition();
        }
    }

    protected override void BeginCameraUseWithFixedPoints(PlayerComponents playerComponents, Transform cameraTransform, Vector3 targetCamPos, Quaternion targetCamRot, bool applytoLocalPosition, bool applyToLocalRotation, float duration)
    {
        _initialCameraLocalPosition = playerComponents.CameraPosition.localPosition;
        base.BeginCameraUseWithFixedPoints(playerComponents, cameraTransform, targetCamPos, targetCamRot, applytoLocalPosition, applyToLocalRotation, duration);
        _isTargetInside = true;
    }

    protected override void EndCameraUseWithDynamicPoints(Transform targetPosition, Quaternion targetRotation, Vector3 extraPosOffset, float duration)
    {
        base.EndCameraUseWithDynamicPoints(targetPosition, targetRotation, extraPosOffset, duration);
