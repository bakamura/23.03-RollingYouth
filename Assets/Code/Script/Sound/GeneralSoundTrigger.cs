...
public class GeneralSoundTrigger : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            float soundRange = Vector3.Distance(other.attachedRigidbody.position, other.GetComponent<PlayerComponents>().Camera.position);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (_playSoundOnCollisionEnter)
        {
            ...
            float soundRange = Vector3.Distance(collision.GetContact(0).point, collision.collider.GetComponent<PlayerComponents>().Camera.position);
